...
func start_game(tables []table, bingo []int) (table, int) {
	for _, called := range bingo {
		isWinner, winnerTable, luckyNum := game_turn(&tables, called)
		if isWinner {
			return winnerTable, luckyNum
		}
	}
	return table{}, 0
}

func game_turn(tables *[]table, called int) (bool, table, int) {
		...
		for row_index, row := range player.rows {
			for point_index, num := range row {
				if num.num == called && !num.isMarked {
					...
					if player.row_points[row_index] == 5 {
						return true, player, called
					}
					if player.column_points[point_index] == 5 {
						return true, player, called
					}
				}
			}
